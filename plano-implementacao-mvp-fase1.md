...
| **M3 — OpenAlex + consolidação** ✅ | Coletor F4; fusão light (§6.5)                                                                                                                        | ✅ **9.941 works** (tipo 'work', com DOI/resumo/tópicos), **4.825 pesquisadores** com última afiliação UEG; base com **8.618 pessoas (3.074 com ORCID)**; **A6 verificado**: 923 pessoas presentes em 2 fontes e 63 em 3, consolidadas por ORCID/Lattes/e-mail exatos (ex.: João Carlos Nabout — 15 docs BDTD + 132 works OpenAlex no mesmo registro); 4 botões ativos. Notas §12.1 | concluído 06/08 |
| **M4 — Busca básica** ✅         | Embeddings locais multilíngues via fastembed; busca híbrida no Streamlit: tema → resultados ranqueados com fonte                                              | ✅ **13.200/13.200 vetores** (12.445 documentos + 70 labs + 685 projetos); **A1 verificado com índice 100%**: busca "Cerrado" retorna teses RENAC/TECCER, dissertações e artigos ranqueados em sub-seções próprias + projetos de extensão (CKAN) + laboratórios com prédio/sala (ueg.br), todos com link da fonte e pessoas vinculadas (optout respeitado); busca quente ~0,5 s (tsvector em coluna gerada + GIN). Decisões §12.3 e §12.4                                                                                                             | concluído 07/08     |
| **M5 — Assistente inteligente** 🔄 | Wizard de tags, chat refinador, upload de roteiro + entendimento LLM                                                                                  | 🔄 Em andamento (07/08): página 🧭 Assistente (3 passos: tema/roteiro → revisão → busca), extração PDF/TXT/DOCX, entendimento por **API online** (DeepSeek padrão — decisão §12.5; Ollama só opção) com fallback heurístico sem IA, Cabine mínima funcional para colar chaves, log `roteiro_anexado`. Falta: teste com chave real + A2 com roteiro do CriaLab                                                                                                                                                                                                                     | \~2 semanas     |
...
### 12.2 Decisões operacionais registradas no M3 (06/08/2026) — revisar se discordar

1. **Co-autores externos não viram pessoa** — nos works da UEG, um mesmo artigo pode ter dezenas de co-autores de outras instituições. Só entram no cadastro de personagens os autores cuja *própria autoria* declara afiliação UEG (`authorships[].institutions` contém I3129565396); os demais ficam preservados em `autores_raw`/`metadados` para auditoria, sem registro em `pessoa`. Sem essa regra, o banco encheria de gente sem vínculo real com a UEG.
2. **Polite pool sem e-mail inventado** — o `mailto=` da OpenAlex é lido de `config/instituicoes.yaml` (`polite_pool_mailto`) ou da variável `BD_FILMES_MAILTO`; enquanto o e-mail do CriaLab não for definido (pendência §12.3), a coleta roda no pool comum, que bastou (~5 min para 9.941 works + 4.825 authors). Quando o e-mail existir, é só preencher o config.
3. **Identidade e URL dos works** — `identificador_externo` = ID curto OpenAlex (`W…`); `url` = DOI quando existe, senão a URL OpenAlex; `pdf_url` = melhor localização OA quando disponível. Resumo reconstruído do `abstract_inverted_index`; tópicos OpenAlex em `metadados.topicos` e keywords em `palavras_chave`.
4. **Sujeira conhecida da fonte CKAN** — um registro de cargos veio com o valor "Lattes: Lattes: http://…" (prefixo duplicado no CSV). Mantido como veio (auditoria); limpeza fina fica para a rotina de higienização do M7.

### 12.3 Decisões operacionais registradas no M4 (06/08/2026) — revisar se discordar

1. **Modelo de embeddings: `intfloat/multilingual-e5-large` no lugar do bge-m3** — o plano original previa bge-m3, mas o suporte a ele no fastembed foi mergeado no repositório (PR #602) e **ainda não publicado no PyPI** (última release: 0.8.0). O e5-large multilíngue tem a **mesma dimensão (1024)** — nenhuma alteração de schema —, licença MIT, ~100 línguas e desempenho equivalente para português. Troca futura vira uma variável (`BD_FILMES_MODELO_EMBEDDING`) + reindexação com `--forcar`.
2. **Prefixos obrigatórios do E5** — documentos são indexados com `"passage: "` e consultas com `"query: "` (convenção oficial do modelo; sem isso a qualidade cai). Fica encapsulado em `src/busca/embeddings.py`.
3. **Corte de texto em ~1.300 caracteres (~350 tokens)** — o E5 trunca em 512 tokens; título + palavras-chave + início do resumo carregam o sinal temático para *recuperação*. Medido: o corte reduz pela metade o tempo de CPU nos documentos longos. Se o gerente quiser o texto completo, basta reindexar com `--forcar` após ajustar `MAX_CARACTERES`.
4. **Busca em 3 seções ranqueadas (pesquisas / extensão / laboratórios)** — uma única consulta vetorial, três buscas com filtro de tipo. Sem isso, as teses (textos mais ricos) abafariam projetos e laboratórios no ranking global — e o critério A1 exige as três categorias presentes.
5. **Híbrido de verdade: RRF (k=60) entre vetorial e lexical** — ranking vetorial (cosseno pgvector) + ranking lexical (`tsvector` em português sobre título+resumo+palavras-chave) fundidos por Reciprocal Rank Fusion. O lexical cobre siglas e nomes próprios que o vetor dilui (ex.: "RENAC", "TECCER").
6. **Indexação retomável por lotes de 64 com gravação imediata** — cada lote faz commit no banco; interromper (Ctrl+C, fechar janela, queda de energia) não perde nada e a rotina só processa `embedding IS NULL`. Espelhos em `documento` (fichas de lab e projetos) **herdam o vetor da tabela-entidade** por join — metade do cômputo poupada nessas coleções.
7. **Modelo em `dados/modelos/`** — cache persistente dentro do projeto (fora do git), não na pasta temporária do Windows (que o sistema pode limpar e forçaria novo download de 2,2 GB).
8. **Índices HNSW** (`vector_cosine_ops`) nos três campos vetoriais, criados de forma idempotente ao fim da carga (e já no schema para instalações novas). Com ~13 mil vetores a varredura sequencial já seria rápida; o índice prepara o crescimento.
9. **Carga inicial pelo app ou por `indexar.bat`** — para a primeira carga (1,5–2 h no i7-10700), o `.bat` é o caminho mais robusto (janela preta independente de navegador); o botão na página 🔄 Atualizar dados cobre as atualizações incrementais seguintes e quem preferir a interface.
10. **`busca_log` ativo** — cada busca registra entrada + filtros (sem dados pessoais além do texto digitado) para auditoria e melhoria do ranking.

### 12.4 Decisões operacionais no fechamento do M4 (07/08/2026) — revisar se discordar

1. **Pesquisas divididas em 3 sub-seções (teses · dissertações · artigos)** — com o índice completo, o ranking único de "pesquisas" (11.689 itens) colocava a primeira tese só na posição 29: os 9.941 artigos do OpenAlex abafavam as 31 teses RENAC/TECCER, e o A1 exige teses visíveis. A busca agora roda 5 seções independentes (mesma consulta vetorial, filtro de tipo por seção) — teses sempre aparecem no topo da sua seção. Medido: 5 seções em ~0,46 s (busca quente).
2. **tsvector em coluna gerada + índice GIN** — a fusão RRF calculava `to_tsvector('portuguese', título+resumo+palavras-chave)` dos ~12,4 mil documentos **a cada busca** (impossível indexar cálculo on-the-fly). Agora `documento.tsv` é coluna `GENERATED ALWAYS … STORED` (função `tsv_documento()` marcada IMMUTABLE — `array_to_string` é STABLE no Postgres e barraria a coluna gerada direta) com índice GIN `documento_tsv_gin`. Aplicado no banco vivo e no `schema.sql` (idempotente). Efeito: busca quente caiu de ~3,8 s/seção para ~0,5 s as 5 seções.
3. **Corte de texto sem quebrar palavras** — o limite de 1.300 caracteres (§12.3.3) cortava no meio da palavra, gerando subtokens ruins no tokenizer do e5 (ruído leve no vetor — o e5 usa WordPiece, não vira UNK). Agora o corte recua ao último espaço (`_montar` no `indexar.py` e `preparar_texto` no `embeddings.py`). **Os 13.200 vetores atuais ainda foram gerados com o corte antigo** — reindexação com `indexar.bat --forcar` (~2 h, 1 clique) fica a critério do gerente; o impacto é marginal e o índice misto funciona normalmente.
4. **3 works sem título** (de 9.941) — registros do OpenAlex sem campo título (um deles ranqueia bem em "Cerrado" pelo resumo). Exibidos como "(sem título na fonte — ver link)" com o DOI preservado. Higiene fina (buscar título no DOI) fica para o M7.
5. **M4 fechado sem reindexar** — o fechamento usa os vetores da carga original; a reindexação do item 3 é melhoria incremental independente, não bloqueia o A1 (verificado com os vetores atuais).

### 12.5 Decisões operacionais registradas no M5 (07/08/2026) — revisar se discordar

1. **IA por API online, não local** — o plano (§7.3) previa Ollama local como padrão de fábrica, mas esta máquina é lenta demais para IA local (relato do gerente, que já testou). O padrão passa a ser **DeepSeek** (💰, pt-BR excelente, centavos/análise), com Kimi (roteiros longos) e OpenRouter (Claude/GPT) como alternativas — os três falam protocolo OpenAI, um único cliente (`src/llm/cliente.py`) cobre todos. Ollama continua disponível como opção na Cabine (não como padrão).
2. **Sem chave = modo básico, nunca erro** — se não houver chave (ou a API falhar), o assistente cai automaticamente para o entendimento heurístico (blocos do roteiro + termos fortes por frequência) e avisa na tela. O usuário leigo nunca fica travado.
3. **Cabine mínima antecipada do M6** — para colar a chave sem editar YAML, a ⚙️ Cabine ganhou versão funcional mínima (provedor padrão + chaves + teste de conexão → grava `config/apis.yaml`, fora do git). A Cabine completa (custos por busca, rerank, websearch) continua no M6.
